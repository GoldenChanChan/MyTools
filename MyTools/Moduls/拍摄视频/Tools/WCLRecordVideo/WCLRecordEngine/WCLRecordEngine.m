...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _startCaptureDeviceOrientation = UIDeviceOrientationPortrait;
    }
    return self;
}

#pragma mark - 公开的方法
- (void)stopRecordWhileAppDidInBackGround {
    if (self.isCapturing) {
        [self pauseCapture];
...
//                    self.currentRecordTime = 0;
                    ...
                    CGFloat grees = 0;
                    if (self.startCaptureDeviceOrientation!=UIDeviceOrientationPortrait) {//旋转并保存新的videoPath
                        if (self.startCaptureDeviceOrientation==UIDeviceOrientationLandscapeLeft) {
                            grees = 270;
                        }else if (self.startCaptureDeviceOrientation==UIDeviceOrientationLandscapeRight) {
                            grees = 90;
                        } else {
                            grees = 180;
                        }
                        //旋转
                        AVSERotateCommand *rotateCommand = [[AVSERotateCommand alloc] initWithComposition:nil videoComposition:nil audioMix:nil];
                        AVAsset *asset = [[AVURLAsset alloc] initWithURL:url options:nil];
                        [rotateCommand rotateWithAsset:asset radians:grees];
                        NSLog(@"视频内容旋转~~~~~");
                        //导出到新路径下
                        AVSEExportCommand *exportComman = [[AVSEExportCommand alloc] initWithComposition:rotateCommand.mutableComposition videoComposition:rotateCommand.mutableVideoComposition audioMix:rotateCommand.mutableAudioMix];
                        [exportComman performWithAsset:asset originUrlPath:path complete:^(BOOL succ, NSString *newPath) {
                            if (succ) {
                                NSLog(@"旋转后的内容已保存成功~~~~~url=%@",newPath);
                                self.videoPath = newPath;
                                //保存至相册
                                [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                                    [PHAssetChangeRequest creationRequestForAssetFromVideoAtFileURL:[NSURL URLWithString:newPath]];
...
- (void)movieToImageHandler:(void (^)(UIImage *movieImage))handler {
    ...
    CMTime thumbTime = CMTimeMakeWithSeconds(30, 60);
...
- (void)setStartCaptureDeviceOrientation:(UIDeviceOrientation)startCaptureDeviceOrientation {
    _startCaptureDeviceOrientation = startCaptureDeviceOrientation;
}
...
//    
//    NSString *videoCache = [paths lastObject];
